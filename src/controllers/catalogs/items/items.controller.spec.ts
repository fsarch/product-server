...
describe('ItemsController', () => {
  ...
  describe('List', () => {
    it('resolves itemTypeExternalId filters to item type ids and passes them to ItemService.List', async () => {
      itemTypeService.GetByExternalId.mockResolvedValue({
        id: 'product-item-type-id',
      });

      await controller.List('catalog-id', undefined, undefined, [
        '$system.product',
      ]);

      expect(itemTypeService.GetByExternalId).toHaveBeenCalledWith(
        'catalog-id',
        '$system.product',
      );
